...
# form
    ...
    for form_i in range(len(in_findall)):
        ...
        out_names = [x.strip() for x in out_findall[form_i][0].split(',')]

        parse_si_str = ''
        for item in in_names:
            parse_si_str += f'parse_si({item.strip()}.value),'

        html_out += f'<h3>{fun_name}</h3>\n'
        ...
        for in_i in range(len(in_names)):
            ...
            <tr>
                <td>%s</td>
                <td><input name="%s" maxlength="20" size="20"/></td>
                <td>%s</td>
